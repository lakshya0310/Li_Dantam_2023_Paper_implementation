...
public:
    InfeasibilityPlanner(int dim) : dim_(dim) {
        std::cout  << "Everything good in the constructor" << std::endl;
    }

    // Add samples with custom labels
    void add_sample(const Eigen::VectorXd& q, float& label) {
        std::cout << "Is this ok" << std::endl;
        if (q.size() != dim_) throw std::runtime_error("Dimension Mismatch!\n");
        samples_.push_back(q);
        labels_.push_back(label);
        std::cout << "Everything here is fine" << std::endl;
    }
